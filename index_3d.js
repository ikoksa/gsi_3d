function INCLUDE() {
    var dir = './js.lib/';
    var scripts = new Array('html2canvas.js', 'togeojson.js');
...
CONFIG.DEMAREA3 = ['10/879/406', '10/879/407'];
...
function Init(iFrame) {
    ...
    if (oWin != null) {
        try {
            if (oWin.CONFIG) {
                if (oWin.CONFIG.layerBase && oWin.CONFIG.layerBase.length > 0) {
                    CONFIG.layerBase = oWin.CONFIG.layerBase;
                }
                if (
                    oWin.CONFIG.layerBaseDefaultID &&
                    oWin.CONFIG.layerBaseDefaultID.length > 0
                ) {
                    CONFIG.layerBaseDefaultID = oWin.CONFIG.layerBaseDefaultID;
                }
                if (oWin.CONFIG.layers && oWin.CONFIG.layers.length > 0) {
                    CONFIG.layers = oWin.CONFIG.layers;
                }
                if (oWin.CONFIG.layersTab) {
                    for (var i = 0; i < oWin.CONFIG.layersTab.length; i++) {
                        for (
                            var j = 0;
                            j < oWin.CONFIG.layersTab[i].layers.length;
                            j++
                        ) {
                            CONFIG.layers.push(
                                oWin.CONFIG.layersTab[i].layers[j]
                            );
                        }
                    }
                }
...
function getGSIMapUrl(args) {
    ...
    if (0 < args['ls'].length) {
        ...
        if (args['ls'][0].grayscale) base += '&base_grayscale=1';
    }
...
function InitGet() {
	...
	+ "&a="             + vCameraZ;             ：高さ方向の倍率
        ...
        var reliefData = ret['relief']
            ? GSI3D.ReliefTileLayer.decodeElevationDataText(ret['relief'])
            : '';
        ...
        if (oFaceTransparentCheck)
            oFaceTransparentCheck.prop({ checked: bFaceTransparent });
        ...
        if (
            !ret['frame'] ||
            (ret['frame'] != 'normal' && ret['frame'] != 'none')
        )
            ret['frame'] = 'trans';
        // 引数：DEM
        ...
        $('#display_scaleinfo').html(
            '3Dモデルの大きさ:南北:' + scaleHeight + '東西:' + scaleWidth
        );

        if (ret['tile_n_px_w'] > ret['tile_n_px_h']) {
            nVertexNumX = 256;
            nVertexNumY = Math.floor(
                ret['tile_n_px_h'] * (256 / ret['tile_n_px_w'])
            );
        } else {
            nVertexNumY = 256;
            nVertexNumX = Math.floor(
                ret['tile_n_px_w'] * (256 / ret['tile_n_px_h'])
            );
        }
        ...
        if (fDemTrimFromCenter) {
            ...
            ret['lon_lt_x'] = nCX_RangeS;
            ret['lon_lt'] = nCX_RangeS_Lon;
            ret['lon_rb_x'] = nCX_RangeE;
            ret['lon_rb'] = nCX_RangeE_Lon;
            ret['lat_lt_y'] = nCY_RangeS;
            ret['lat_lt'] = nCY_RangeS_Lat;
            ret['lat_rb_y'] = nCY_RangeE;
            ret['lat_rb'] = nCY_RangeE_Lat;
            ...
            if (ret['tile_n_px_w'] % 256 != 0) {
                ret['trim_n_px_x'] =
                    (ret['tile_n_px_w'] - ret['trim_n_x'] * 256) * 0.5;
            } else {
                ret['trim_n_px_x'] = 0;
            }
            ...
            if (ret['tile_n_px_h'] % 256 != 0) {
                ret['trim_n_px_y'] =
                    (ret['tile_n_px_h'] - ret['trim_n_y'] * 256) * 0.5;
            } else {
                ret['trim_n_px_y'] = 0;
            }
            ret['trim_x_s'] += vT_X;
            ...
            if (fCalxX) {
                ret['lon_rb'] = GetTile2Lng(
                    ret['lon_lt_x'] + ret['tile_n_w'] - 1,
                    ret['z']
                );
                ret['lon_rb_x'] = GetTileX(ret['z'], ret['lon_rb']).n;
            }
            if (fCalxY) {
                ret['lat_rb'] = GetTile2Lat(
                    ret['lat_lt_y'] + ret['tile_n_h'] - 1,
                    ret['z']
                );
        ...
        ret['distance'] = CalcLatitudinallyDistance(
            ret['lat'],
            ret['z'],
            Math.max(parseInt(ret['w']), parseInt(ret['h']))
        );
...
var InitLoadLayersTxt_Proc_DataSrcSet = function (layer) {
    if (layer.src) {
        if (layer.src.indexOf('./') == 0) {
            var path = layer.src_url.substring(
                0,
                layer.src_url.lastIndexOf('/')
            );
...
var InitLoadLayersTxt_Proc_Success = function (data) {
    ...
    if (_Load_DataIndex == 0) {
        var json_base = JSON.parse(
            '{ "layers": [ { "type": "LayerGroup", "title": "", "title_sys": "", "iconUrl": "", "open": false, "toggleall": false, "entries": [] } ] }'
        );
    ...
    if (json.layers) {
        for (var i = 0; i < json.layers.length; i++) {
            InitLoadLayersTxt_Proc_Success_SRC(
                json.layers[i],
                _Load_Data[_Load_DataIndex].fname
            );
        }
    }
...
var InitLoadLayersTxt_Proc_Success_SRC = function (layer, url) {
    ...
    if (
        layer.type == 'Layer' ||
        (layer.type == 'LayerGroup' && layer.id != '')
    ) {
            ...
            for (n = 0; n < args['ls'].length; n++) {
                ...
                for (var childNo = 0; childNo < entries.length; childNo++) {
                    ...
                    if (d) {
                        ...
                        var dUrlType = InitLoadLayersTxt_ProcSrc_URL2LayerType(
                            d.url
                        );
                        var dUrlStyle = false;
                        var fTileUrl = true;
                        ...
                        if (!dUrlStyle || !_Load_StyleZoom) {
                            if (d.minZoom) {
                                if (parseInt(args['z']) < parseInt(d.minZoom)) {
                                    fTileUrl = false;
                                }
                            }
                            if (d.maxZoom) {
                                if (parseInt(args['z']) > parseInt(d.maxZoom)) {
                                    fTileUrl = false;
                                }
                            }
                        }
...
var InitLoadLayersTxt_ProcSrc_Success = function (data) {
    ...
    for (var i = 0; i < _Load_DataSrc[0].entries.length; i++) {
        ...
        InitLoadLayersTxt_Proc_Success_SRC(
            _Load_DataSrc[0].entries[i],
            _Load_DataSrc[0].entries[i].src_url
        );
...
function InitFrame(o) {
    if (o != null) {
        ...
        o.appendChild(oFrame);
        oFrame.style.display = 'none';
        oFrame3D = document.createElement('div');
        oFrame.appendChild(oFrame3D);
        {
...
function InitFrameDownload(o) {
    if (o != null) {
        oFrame3D_Download = document.createElement('div');
        oFrame.appendChild(oFrame3D_Download);
        ...
        var setCameraPositionEastButton = $('<a>')
            .addClass('camera-east')
            .attr({ href: 'javascript:void(0);' })
            .click(function () {
                setCameraPosition('e');
            });
        var setCameraPositionWestButton = $('<a>')
            .addClass('camera-west')
            .attr({ href: 'javascript:void(0);' })
            .click(function () {
                setCameraPosition('w');
            });
        var setCameraPositionNorthButton = $('<a>')
            .addClass('camera-north')
            .attr({ href: 'javascript:void(0);' })
            .click(function () {
                setCameraPosition('n');
            });
        var setCameraPositionSouthButton = $('<a>')
            .addClass('camera-south')
            .attr({ href: 'javascript:void(0);' })
            .click(function () {
                setCameraPosition('s');
            });
        var setCameraPositionResetButton = $('<a>')
            .addClass('camera-reset')
            .attr({ href: 'javascript:void(0);' })
            .click(function () {
                setCameraPosition();
            });
        ...
        oFaceTransparentCheck = $('<input>')
            .attr({ type: 'checkbox', id: 'facetrans_check' })
        ...
        oDisplayInfoCheck = $('<input>')
            .attr({ type: 'checkbox', id: 'displayinfo_check' })
        ...
        var displayInfoLabel = $('<label>')
            .attr({ for: 'displayinfo_check' })
            .html('方位・大きさの表示');
...
function InitLoadLayers() {
    ...
    oLayersTM = setInterval(function () {
        if (vLayersTM_Cur >= vLayersTM_Max) {
            clearInterval(oLayersTM);
            InitProgressMsgInfo(
                'タイムアウトしました([' + vLayersTM_Max / 1000 + ']秒)'
            );
        } else {
            vLayersTM_Cur += vLayersTM;
            ...
            if (vVectors == 0 || vVectors == vVectorsN) {
                ...
                if (
                    ...
                    if (oTextureCanvas != null) {
                        ...
                        var imageData = oTextureCanvas_2D.getImageData(
                            args['trim_x_s'],
                            args['trim_y_s'],
                            vTextureCanvas_W,
                            vTextureCanvas_H
                        ...
                        oTextureCanvas_2D.putImageData(
                            imageData,
                            0,
                            0,
                            0,
...
function GetScaleTileSize(z, zNative) {
    return (GetScale(z) / GetScale(zNative)) * 256;
}

function GetTileX(z, lon) {
...
function GetTileN(z, zN, x, y) {
    var nR = Math.pow(2, z - zN);
    var nX = Math.floor(x / nR);
    var nY = Math.floor(y / nR);
    return { x: nX, y: nY };
}
...
function CalcLatitudinallyDistance(lat, z, pxsize) {
    R = 6378137.0;
    return (
        ((2 * Math.PI * R * Math.cos((lat / 180.0) * Math.PI)) /
            Math.pow(2, z) /
            256) *
        pxsize
    );
...
function ConverUnit(lat, z, radius, unit_src, unit_to) {
    if (unit_src == 'm' || unit_src == 'km') {
        unit_src = 'm';
    }
    if (unit_to == 'm' || unit_to == 'km') {
        unit_to = 'm';
    }

    var mppx =
        Math.abs(Math.cos((lat / 180) * Math.PI) * 2 * Math.PI * 6378137) /
        Math.pow(2, z + 8);
    if (unit_src == 'm' && unit_to == 'px') {
        radius = Math.floor(radius / mppx);
    } else if (unit_src == 'px' && unit_to == 'm') {
        radius = Math.floor(radius * mppx);
    }
...
function RequestLayers(url, z, x, y, nTilesOTS_X, nTilesOTS_Y) {
    ...
    for (var nLayers = 0; nLayers < vLayers.length; nLayers++) {
        ...
        if (vURLType == 'tile') {
            if (vURLI.length == 2) {
                if (!vURLStyle) vURLStyle = vURLI[0] + 'style.js';
                z = vLayers[nLayers].z;
                x = vLayers[nLayers].x;
                y = vLayers[nLayers].y;
                    ...
                    $.ajax({
                        ...
                        .done(function (data, status, jqXHR) {
                            if (data == null) {
                                ...
                                try {
                                    vLayersData_VectorAjax.data = eval(
                                        '(' + data + ')'
                                    );
                                    RequestLayersVectorStyle(
                                        vLayersData_VectorAjax,
                                        vLayersData_VectorAjax.data
                                    );
                                } catch (e) {
                                    InitProgressMsgError(
                                        'VectorStyle[' +
                                            vLayersData_VectorAjax.src +
                                            ']...[' +
                                            e +
                                            ']'
                                    );
        ...
        for (var id in vLayersData) {
            if (vLayersData.hasOwnProperty(id)) {
                ...
                for (var nLayers = 0; nLayers < dLayers.length; nLayers++) {
                    if (
                        dLayers[nLayers].type == 'vector' &&
                        !dLayers[nLayers].load
                    ) {
                        ...
                        if (vLayersData_VectorStyle) {
                            if (
                                vLayersData_VectorStyle.hasOwnProperty(
                                    dLayers[nLayers].id
                                )
                            ) {
                                var zoom_min =
                                    vLayersData_VectorStyle[id].zoom_min;
                                var zoom_max =
                                    vLayersData_VectorStyle[id].zoom_max;
                                if (zoom_min != null) {
                                    if (args['z'] < zoom_min) {
                                        fRequest = false;
                                    }
        ...
        if (data.options) {
            if (_Load_StyleZoom) {
                if (data.options.minZoom) {
                    vLayersData_Vector.zoom_min = data.options.minZoom;
                }
                if (data.options.maxZoom) {
                    vLayersData_Vector.zoom_max = data.options.maxZoom;
                }
                if (data.options.maxNativeZoom) {
                    vLayersData_Vector.zoom_native = data.options.maxNativeZoom;
                }
            }
        }
    }
                                    ...
                                    try {
                                        ...
                                        if (
                                            ...
                                            if (vStyle.geojsonOptions.style) {
                                                try {
                                                    var vOptions = vStyle.geojsonOptions.style(
                                                        vDataFeatures[n]
                                                    );
                                                    if (vOptions) {
                                                        ...
                                                        if (
                                                            ...
                                                            data.features[
                                                                n
                                                            ].properties._color = RequestLayersVectorStyleSetColorToHex(
                                                                vOptions.color
                                                            );
                                                        }
                                                        if (
                                                        ...
                                                        if (
                                                            ...
                                                            data.features[
                                                                n
                                                            ].properties._fillColor = RequestLayersVectorStyleSetColorToHex(
                                                                vOptions.fillColor
                                                            );
                                                        }
                                                        if (
                                                            vOptions.fillOpacity !=
                                                            null
                                                        ) {
                                                            data.features[
                                                                n
                                                            ].properties._fillOpacity =
                                                                vOptions.fillOpacity;
                                                        }
                                            ...
                                            if (
                                                ...
                                                try {
                                                    vDataFeatures[
                                                        ...
                                                        'gsi.3d';
                                                    var vMarker = vStyle.geojsonOptions.pointToLayer(
                                                        vDataFeatures[n],
                                                        vDataFeatures[n]
                                                            .geometry
                                                            .coordinates
                                                            ...
                                                            if (
                                                                ...
                                                                if (fMarker) {
                                                                    ...
                                                                    if (
                                                                        ...
                                                                        data.features[
                                                                            n
                                                                        ].properties._iconSize = new Array(
                                                                            2
                                                                        );
                                                                        data.features[
                                                                            n
                                                                        ].properties._iconSize[0] =
                                                                            vMarker.iconSize[0];
                                                                        data.features[
                                                                            n
                                                                        ].properties._iconSize[1] =
                                                                            vMarker.iconSize[1];
                                                                    }
                                                                    if (
                                                                        vMarker.iconAnchor &&
                                                                        ...
                                                                        data.features[
                                                                            n
                                                                        ].properties._iconAnchor = new Array(
                                                                            2
                                                                        );
                                                                        data.features[
                                                                            n
                                                                        ].properties._iconAnchor[0] =
                                                                            vMarker.iconAnchor[0];
                                                                        data.features[
                                                                            n
                                                                        ].properties._iconAnchor[1] =
                                                                            vMarker.iconAnchor[1];
                                                                    }
                                                                }
                                                            }
                                                            // (Circle, CircleMarker)
...
CanvasRenderingContext2D.prototype.fillTextVertical = function (
    ...
    CanvasRenderingContext2D.prototype._Text = function (text) {
        ...
        _CL.push(
            this._TextData(_CL, 'Bracket', '‘“（〔［｛〈《「『【⦅〘〖«〝', 1.3)
        );
        _CL.push(
            this._TextData(_CL, 'Bracket', '’”）〕］｝〉》」』】⦆〙〗≫〟', 1)
        );
                ...
                for (var n = 1; n < _CL.length; n++) {
                    if (pos < _CL[n].p) {
                        ...
                        if (o.t == 'Plane') {
                            ...
                            ret.push(this.Text_TypeBracket(o.arg));
                        } else if (o.t == 'Period') {
                            ret.push(this.Text_TypePeriod());
                        } else if (o.t == 'TopRight') {
                            ret.push(this.Text_TypeTopRight());
                        } else if (o.t == 'Rotate90') {
                            ret.push(this.Text_TypeRotate90());
                        }
                        fCL = true;
                        break;
                    }
    ...
    CanvasRenderingContext2D.prototype._TextData = function (
        _CL,
        type,
        text,
        arg
    ...
    CanvasRenderingContext2D.prototype.Text_TypePlane = function () {
        return new _text_t(this._text_c);
    };
    CanvasRenderingContext2D.prototype.Text_TypePeriod = function () {
        return new _text_t(this._text_c, { t_Period: true });
    };
    CanvasRenderingContext2D.prototype.Text_TypeBracket = function (rate) {
        return new _text_t(this._text_c, { t_Rotate90: true, v_RHeight: rate });
    };
    CanvasRenderingContext2D.prototype.Text_TypeTopRight = function () {
        return new _text_t(this._text_c, { t_TopRight: true });
    };
    CanvasRenderingContext2D.prototype.Text_TypeRotate90 = function () {
        return new _text_t(this._text_c, { t_Rotate90: true });
    };
    ...
    CanvasRenderingContext2D.prototype.proc = function (text, x, y, deg) {
        if (deg < -360 || deg > 360) {
            ...
            deg =
                (Math.abs(deg) - Math.floor(Math.abs(deg) / 360) * 360) *
                deg_sign;
        }
        if (deg == 360) {
            deg = 0;
                ...
                for (nx_dem = 0; nx_dem < 256; nx_dem++) {
                    ...
                    if (demData) {
                        ...
                        if (vTilesDemAryV == null) {
                            ...
                            vTilesDemAryV =
                                demData[256 * ny_dem + nx_dem] + 300.0; //300.0というのはかさ上げしたい高さ(最高の高さに合わせる)
                        }
                    }
    ...
    for (var nLayers = 0; nLayers < vLayers.length; nLayers++) {
        ...
        if (vURLType == 'tile') {
            ...
            strRgx = vURL
                ...
                .replace('{x}', '\\d+')
                .replace('{y}', '\\d+');
            var rgxZfromURL = new RegExp(strRgx); //190529
            strRgx = vURL
                .split('?')[0]
                .replace('{z}', '\\d+')
                .replace('{x}', '(\\d+)')
                .replace('{y}', '\\d+');
            var rgxXfromURL = new RegExp(strRgx); //190529
            strRgx = vURL
                .split('?')[0]
                .replace('{z}', '\\d+')
                .replace('{x}', '\\d+')
                .replace('{y}', '(\\d+)');
            var rgxYfromURL = new RegExp(strRgx); //190529
            ...
            for (var i = 0; i < vLayer.length; i++, ny_tile++) {
                ...
                if (!vLayer[i].loadCanvas) {
                    ...
                    if (rgxZfromURL.test(imgURL)) {
                        zz = parseInt(RegExp.$1);
                    }
                    if (rgxXfromURL.test(imgURL)) {
                        xx = parseInt(RegExp.$1);
                    }
                    if (rgxYfromURL.test(imgURL)) {
                        yy = parseInt(RegExp.$1);
                    }
                    ...
                    if (vURLExt == 'img') {
                        if (vID == CONFIG.FREERELIEFID) {
                            ...
                            if (demData) {
                                var hillshadeMapImage = demManager.getHillshademapImage(
                                    vLayer[i].coords.x,
                                    vLayer[i].coords.y,
                                    vLayer[i].coords.z
                                );
                                freeReliefTileDrawer.draw(
                                    tileCanvas,
                                    demData,
                                    hillshadeMapImage
                                );
            ...
            if (vLayer.length > 0) {
                if (!vLayer[0].loadCanvas) {
                    ...
                    try {
                        if (vURLExt == 'geojson' || vURLExt == 'kml') {
                            var data = null;

                            if (vURLExt == 'geojson') {
                                data = LoadLayersProcVectorData(vLayer[0]);
                            } else if (vURLExt == 'kml') {
                                data = LoadLayersProcVectorDataKML(
                                    vLayer[0].data
                                );
                            }
                            if (data && data != null) {
...
function LoadLayersProcCall() {
    LoadLayersProc(
        vLoadLayersProc_oTextureCanvas_2D,
        vLoadLayersProc_x,
        vLoadLayersProc_y,
        vLoadLayersProc_wTileImg,
        vLoadLayersProc_hTileImg
    );
}
        ...
        if (data.features) {
            for (var n = 0; n < data.features.length; n++) {
                ...
                if (data.features[0].geometry && data.features[0].properties) {
                    ...
                    if (data_type) {
                        var data_styleUrl =
                            data.features[n].properties.styleUrl;
                        if (data_styleUrl && data_styleUrl != '') {
                            var data_style_id = data_styleUrl.replace(
                                /\./g,
                                '\\.'
                            );
                            ...
                            if (data_style.length >= 0) {
                                ...
                                if (data_style) {
                                    ...
                                    if (
                                        ...
                                        data.features[
                                            n
                                        ].properties._markerType = _markerType;
                                        LoadLayersProcVectorDataKML_properties(
                                            data.features[n],
                                            data_style
                                        );
                                    }
...
function LoadLayersProcVectorDataKML_properties(data, data_style) {
    ...
    if (data.properties._markerType == 'Icon') {
        ...
        if (_icon) {
            ...
            data.properties._iconScale = data_style
                .find('IconStyle')
                .find('scale')
                .text();
...
function LoadLayers_Vectors(oCanvas, oData, vUrl) {
    ...
    try {
        ...
        var vRange_Lon_L = args['lon_lt'];
        var vRange_X_L = args['lon_lt_x'];
        var vRange_Lon_R = args['lon_rb'];
        var vRange_X_R = args['lon_rb_x'];
        var vRange_Lat_T = args['lat_lt'];
        var vRange_Y_T = args['lat_lt_y'];
        var vRange_Lat_B = args['lat_rb'];
        var vRange_Y_B = args['lat_rb_y'];
        ...
        var vDeg2PxY =
            vTextureCanvas_H /
            (LoadLayers_Vectors_MercaY(vRange_Lat_T, vZ) -
                LoadLayers_Vectors_MercaY(vRange_Lat_B, vZ));
        ...
        for (var n = 0; n < oData.length; n++) {
            ...
            if (vData._hasZdata) {
                ...
                if (
                    vLatLngBounds.lt.lng <= vData._bounds.rb.lng &&
                    ...
                    if (vData.properties._markerType == 'Circle') {
                        ...
                        if (vData.geometry['type'] == 'Polygon') {
                            for (
                                var polyNum = 0;
                                polyNum < vData.geometry.coordinates.length;
                                polyNum++
                            ) {
                                var lineData = {
                                    type: 'Feature',
                                    geometry: {
                                        type: 'LineString',
                                        coordinates:
                                            vData.geometry.coordinates[polyNum],
                                    },
                                    properties: $.extend(
                                        true,
                                        [],
            ...
            try {
                ...
                if (vDataType == 'Point' || vDataType == 'MultiPoint') {
                    ...
                    for (
                        var nGeometryMulti = 0;
                        nGeometryMulti < nGeometryMax_Multi;
                        nGeometryMulti++
                    ) {
                        ...
                        if (vDataGeometryArray.length >= 2) {
                            /*-------------------------------------------------------------------------------------------------------*/
                            if (
                                vData.properties['_markerType'] == 'Font' ||
                                (vData.properties['_text_fontText'] != null &&
                                    vData.properties['_text_fontText'] != '')
                            ) {
                                var iDataName = '';
                                ...
                                var vDataGeometryLon = parseFloat(
                                    vDataGeometryArray[0]
                                );
                                var vDataGeometryLat = parseFloat(
                                    vDataGeometryArray[1]
                                );

                                var vX = LoadLayers_Vectors_2PX(
                                    vDataGeometryLon,
                                    vRange_Lon_L,
                                    vDeg2PxX,
                                    'lon'
                                );
                                var vY = LoadLayers_Vectors_2PX(
                                    vDataGeometryLat,
                                    vRange_Lat_T,
                                    vDeg2PxY,
                                    'lat'
                                ...
                                var vTextFontFamily =
                                    vData['properties'][
                                        iDataName + '_fontFamily'
                                    ];
                                var vTextFontColor =
                                    vData['properties'][
                                        iDataName + '_fontColor'
                                    ];
                                var vTextFontWriteMode =
                                    vData['properties'][
                                        iDataName + '_fontWriteMode'
                                    ];
                                var vRotate = parseFloat(
                                    vData['properties'][iDataName + '_rotate']
                                );
                                var vShadow =
                                    vData['properties'][iDataName + '_Shadow'];
                                var vShadowSize =
                                    vData['properties'][
                                        iDataName + '_Shadow_size'
                                    ];
                                var vShadowFontColor =
                                    vData['properties'][
                                        iDataName + '_Shadow_fontColor'
                                    ];
                                ...
                                for (var nText = 0; nText < nTextMax; nText++) {
                                    ...
                                    if (vTextFontWriteMode == 'vertical') {
                                        oCanvas.fillTextVertical(
                                            vText,
                                            vX - vXA,
                                            vY - vYA + vTextFontSize,
                                            vRotate
                                        );
                                    } else {
                                        oCanvas.save();
                            ...
                            if (vData.properties['_markerType'] == 'DivIcon') {
                                ...
                                var vXA = 0;
                                if (
                                    vData['properties']['_iconAnchor'] != null
                                ) {
                                    vXA = parseInt(
                                        vData['properties']['_iconAnchor'][0],
                                        10
                                    );
                                }
                                var vYA = 0;
                                if (
                                    ...
                                    vYA = parseInt(
                                        vData['properties']['_iconAnchor'][1],
                                        10
                                    );
                                }
                                ...
                                if (vVectorPointDivIcon == 'HTML2CANVAS') {
                                    ...
                                    if (vVectorHTML.o == null) {
                                        ...
                                        vVectorHTML.oDiv = document.createElement(
                                            'div'
                                        );
                                        vVectorHTML.oDiv.style.display =
                                            'block';
                                        vVectorHTML.oDiv.style.position =
                                            'absolute';
                                        vVectorHTML.oDiv.style.top =
                                            -vVectorPointDivIcon_SizeW -
                                            10 +
                                            'px';
                                        vVectorHTML.oDiv.style.left =
                                            -vVectorPointDivIcon_SizeH -
                                            10 +
                                            'px';
                                        vVectorHTML.o.appendChild(
                                            vVectorHTML.oDiv
                                        );
                                        vVectorHTML.oIFrame = document.createElement(
                                            'iframe'
                                        );
                                        vVectorHTML.oIFrame.style.width =
                                            vVectorPointDivIcon_SizeW + 'px';
                                        vVectorHTML.oIFrame.style.height =
                                            vVectorPointDivIcon_SizeH + 'px';
                                        vVectorHTML.oDiv.appendChild(
                                            vVectorHTML.oIFrame
                                        );
                                    }
                                    ...
                                    var vTextSize =
                                        vRPX *
                                        vVectorPointDivIcon_StyleFontSize;

                                    oCanvas.beginPath();
                                    oCanvas.font =
                                        vVectorPointDivIcon_StyleFontWeight +
                                        ' ' +
                                        vTextSize +
                                        'px ' +
                                        vVectorPointDivIcon_StyleFontFamily;
                            ...
                            if (vData.properties['_markerType'] == 'Icon') {
                                ...
                                var vW = 10;
                                if (vData['properties']['_iconSize'] != null) {
                                    vW = Math.floor(
                                        vRPX *
                                            parseInt(
                                                vData['properties'][
                                ...
                                var vScale = 1;
                                if (vData['properties']['_iconScale'] != null) {
                                    vScale = parseFloat(
                                        vData['properties']['_iconScale']
                                    );
                                }
                                ...
                                oImg.alt =
                                    ...
                                    vImgOpacity;
                                oImg.style.width =
                                    Math.floor(vW * vScale) + 'px';
                                oImg.style.height =
                                    Math.floor(vH * vScale) + 'px';
                                oImg.onload = function () {
                                    ...
                                    if (vPos.length == 3) {
                                        ...
                                        var vPosY = vPos[1];
                                        var vOpacity = vPos[2];
                                        var vW = this.style.width.replace(
                                            'px',
                                            ''
                                        );
                                        var vH = this.style.height.replace(
                                            'px',
                                            ''
                                        );
                                ...
                                oImg.src = vURL.replace(
                                    'cyberjapandata.gsi.go.jp',
                                    'maps.gsi.go.jp'
                                );
                                if (
                                    oImg.src.indexOf('//maps.gsi.go.jp/') != -1
                                ) {
                                    oImg.src = oImg.src.replace(
                                        'https://',
                                        '//'
                                    );
                                    oImg.src = oImg.src.replace(
                            ...
                            /*-------------------------------------------------------------------------------------------------------*/
                            if (
                                vData.properties['_markerType'] == 'Circle' ||
                                vData.properties['_markerType'] ==
                                    'CircleMarker'
                            ) {
                                var vDataGeometryLon = parseFloat(
                                    vDataGeometryArray[0]
                                ...
                                if (
                                    vData['properties']['_fillColor'] != null &&
                                    vData['properties']['_fillOpacity'] != null
                                ) {
                                    var vStyleColor = LoadLayers_Vevtors_PropertiesColor(
                                        vData['properties']['_fillColor']
                                    );
                                    var vStyleOpacity = parseFloat(
                                        vData['properties']['_fillOpacity']
                                    );
                                ...
                                if (
                                    vData['properties']['_color'] != null &&
                                    vData['properties']['_opacity'] != null
                                ) {
                                    var vStyleColor = LoadLayers_Vevtors_PropertiesColor(
                                        vData['properties']['_color']
                                    );
                                    var vStyleOpacity = parseFloat(
                                        vData['properties']['_opacity']
                                    );
                                    var vStyleWeight = parseInt(
                                        vData['properties']['_weight']
                                    );
                    ...
                    vDataType == 'LineString' ||
                    vDataType == 'MultiLineString'
                ) {
                    var vDataGeometry = vData['geometry']['coordinates'];
                        ...
                        for (var nFig = nFigIni; nFig < 2; nFig++) {
                            ...
                            for (
                                ...
                                if (vDataType == 'Polygon') {
                                    ...
                                    nGeometryMax =
                                        vDataGeometry[nGeometryMulti].length;
                                }
                                for (
                                    var nGeometry = nGeometryMax;
                                    nGeometry != 0;
                                    nGeometry--
                                ) {
                                    var vDataGeometryArray = vDataGeometry;
                                    ...
                                    for (
                                        var nDataGeometry = 0;
                                        nDataGeometry <
                                        vDataGeometryArray.length;
                                        nDataGeometry++
                                    ) {
                                        if (
                                            vDataGeometryArray[nDataGeometry]
                                                .length >= 2
                                        ) {
                                            var vDataGeometryLon = parseFloat(
                                                vDataGeometryArray[
...
function circleToLatLngs(data) {
    ...
    for (var i = 1; i < numSides; i++) {
        ...
        var x =
            ...
            Math.asin(
                Math.sqrt(
                    (Math.pow(Math.sin(dmax_lat / 2), 2) -
                        Math.pow(Math.sin(y / 2), 2)) /
                        (Math.cos(center_lat_rad + y) *
...
function LoadLayers_Vectors_HTML() {
                        ...
                        onrendered: function (canvas) {
                            ...
                            oImg.onload = function () {
                                ...
                                if (vPos.length == 3) {
                                    ...
                                    vVectorHTML.oCanvas.drawImage(
                                        this,
                                        vPosX,
                                        vPosY
                                    ); //, vVectorPointDivIcon_SizeW, vVectorPointDivIcon_SizeH);
    ...
    if (!ret) {
        if (vVectorHTML.o != null) {
            vVectorHTML.oDiv.removeChild(vVectorHTML.oIFrame);
            vVectorHTML.oIFrame = null;
            vVectorHTML.o.removeChild(vVectorHTML.oDiv);
            vVectorHTML.oDiv = null;
            vVectorHTML.o = null;
        }
    }
...
function LoadLayers_DemNormarize(v, nTilesX, nTilesY, pxTile) {
    ...
    for (ny = 0; ny <= nVertexNumY; ny++) {
        for (nx = 0; nx <= nVertexNumX; nx++) {
            ...
            if (nx == nVertexNumX) {
                ret[nz] = ret[ny * (nVertexNumX + 1) + nx - 1];
            } else if (ny == nVertexNumY) {
                ret[nz] = ret[(ny - 1) * (nVertexNumX + 1) + nx];
            } else {
                if (false) {
                    // nVertexNumX == nVertexNumY )
...
function LoadScene() {
    ...
    if (args['tile_n_px_w'] > args['tile_n_px_h']) {
        nGeomSizeX = 100;
        nGeomSizeY = Math.floor(
            args['tile_n_px_h'] * (100 / args['tile_n_px_w'])
        );
    } else {
        nGeomSizeY = 100;
        nGeomSizeX = Math.floor(
            args['tile_n_px_w'] * (100 / args['tile_n_px_h'])
        );
    }
    // メッシュ状ジオメトリを作成する。
...
function getBoxLatLngBounds(center, zoom, w, h) {
    ...
    var pointToLatLng = function (point, zoom) {
        ...
        var lat =
            (2 * Math.atan(Math.exp(untransformedPoint.y)) - Math.PI / 2) * d;
...
function _latLngToPointData(viewBox, lat, lng, h) {
	...
	*/
    ...
    var z =
        (h - parseFloat(vSceneMesh_ZMin)) *
        vSceneMeshDistanceRate *
        vSceneMeshZRate;
...
function polyToriangles(pointArr) {
    ...
    while (points.length > 3) {
        ...
        var triangle = [
            {
                lng: points[idx - 1 >= 0 ? idx - 1 : l - 1].lng,
                lat: points[idx - 1 >= 0 ? idx - 1 : l - 1].lat,
                h: points[idx - 1 >= 0 ? idx - 1 : l - 1].h,
            },
            { lng: points[idx].lng, lat: points[idx].lat, h: points[idx].h },
            {
                lng: points[idx + 1 < l ? idx + 1 : 0].lng,
                lat: points[idx + 1 < l ? idx + 1 : 0].lat,
                h: points[idx + 1 < l ? idx + 1 : 0].h,
            },
        ];
...
function _getFarPoint(p0, points) {
    ...
    for (var i = 0; i < points.length; i++) {
        ...
        var distance = Math.sqrt(
            Math.pow(p0.lng - p.lng, 2) + Math.pow(p0.lat - p.lat, 2)
        );
...
function Draw3DGEOData() {
    ...
    for (var i = 0; i < oGeo3DData.length; i++) {
        if (
            oGeo3DData[i].properties._markerType == 'Polygon' ||
            oGeo3DData[i].geometry['type'] == 'Polygon'
        ) {
            var geometry = new THREE.Geometry();
            var triangles = _draw3DGEODataPolygon(
                geometry,
                viewBox,
                oGeo3DData[i]
            );
            ...
            oGeo3DData[i].properties._fillOpacity =
                oGeo3DData[i].properties._fillOpacity ||
                oGeo3DData[i].properties._fillOpacity == 0
                    ? oGeo3DData[i].properties._fillOpacity
                    : 0.2;
            ...
            oGeo3DData[i].properties._markerType == 'LineString' ||
            oGeo3DData[i].geometry['type'] == 'LineString'
        ) {
            var geometry = new THREE.Geometry();
            var points = _draw3DGEODataLineString(
                geometry,
                viewBox,
                oGeo3DData[i]
            );
            oGeo3DData[i].properties._opacity =
                oGeo3DData[i].properties._opacity ||
                oGeo3DData[i].properties._opacity == 0
                    ? oGeo3DData[i].properties._opacity
                    : 0.5;
            var color = oGeo3DData[i].properties._color
                ? oGeo3DData[i].properties._color
                : 0x0033ff;
            var weight = oGeo3DData[i].properties._weight
                ? oGeo3DData[i].properties._weight
                : 5;
            ...
            var mesh = new THREE.Line(
                ...
                new THREE.LineBasicMaterial({
                    color: color,
                    linewidth: parseInt(weight),
                    opacity: oGeo3DData[i].properties._opacity,
                    depthWrite: true,
                    transparent: true,
                })
            );
            ...
            if (oGeo3DData[i].properties._altitudeMode == 'relativeToGround') {
                var x =
                    ((latlngPoint.lng - viewBox.lt.lng) /
                        (viewBox.rb.lng - viewBox.lt.lng)) *
                    viewBox.w;
                var y =
                    ((latlngPoint.lat - viewBox.rb.lat) /
                        Math.abs(viewBox.rb.lat - viewBox.lt.lat)) *
                    viewBox.h;

                y = viewBox.h - y;
                latlngPoint.h =
                    parseFloat(latlngPoint.h) + getDEMValue(x, y, viewBox);
            ...
            var vR = ConverUnit(
                parseFloat(args['lat']),
                parseInt(args['z']),
                parseFloat(oGeo3DData[i].properties._radius),
                'm',
                'px'
            );
			...
			*/
            ...
            else div = $(oGeo3DData[i].properties._html);
            ...
            var textShadow =
                div.css('text-shadow') || div.css('-ms-text-shadow') || '';
            if (textShadow) {
            ...
            if (!fontFamily || fontFamily == '')
                fontFamily =
                    "'Meiryo','メイリオ','ヒラギノ角ゴ Pro W3','sans-serif'";
            var text = div.text();
            ...
            var url = oGeo3DData[i].properties._iconUrl.replace(
                /cyberjapandata.gsi.go.jp/,
                'maps.gsi.go.jp'
            );
    ...
    if (coordinates.length <= 1) {
        ...
        for (var i = 0; i < coordinates.length; i++) {
            ...
            if (isRelative) {
                ...
                var y =
                    ((p[1] - viewBox.rb.lat) /
                        Math.abs(viewBox.lt.lat - viewBox.rb.lat)) *
                    viewBox.h;
                y = viewBox.h - y;
...
function _draw3DGEODataPolygon(geometry, viewBox, data) {
    ...
    for (var i = 0; i < data.geometry.coordinates[0].length; i++) {
        ...
        if (isRelative) {
            var x =
                ((p[0] - viewBox.lt.lng) / (viewBox.rb.lng - viewBox.lt.lng)) *
                viewBox.w;
            var y =
                ((p[1] - viewBox.rb.lat) /
                    Math.abs(viewBox.rb.lat - viewBox.lt.lat)) *
    ...
    for (var j = 0; j < triangles.length; j++) {
        ...
        var p2 = _latLngToPointData(
            viewBox,
            points[1].y,
            points[1].x,
            points[1].__h
        );
        var p3 = _latLngToPointData(
            viewBox,
            points[2].y,
            points[2].x,
            points[2].__h
        ...
        for (n = 1; n < 5; n++) {
            ...
            for (i = 0; i < geometry.vertices.length; i++) {
                ...
                if (n == 1) {
                    ...
                    y =
                        nGeomSizeY / 2 -
                        (nGeomSizeY / nVertexNumY) * (i % (nVertexNumY + 1));
                } else if (n == 3) {
                    x = nGeomSizeX / 2;
                    y =
                        -(nGeomSizeY / 2) +
                    ...
                    x =
                        nGeomSizeX / 2 -
                        (nGeomSizeX / nVertexNumX) * (i % (nVertexNumX + 1));
                    y = nGeomSizeY / 2;
                }
...
function SceneGeometryZ() {
    ...
    for (ny = 0; ny <= nVertexNumY; ny++) {
        for (nx = 0; nx <= nVertexNumX; nx++) {
            nz = ny * (nVertexNumX + 1) + nx;
            oSceneMesh.geometry.vertices[nz].z =
                (vSceneMesh[nz] - vSceneMesh_ZMin) *
                vSceneMeshDistanceRate *
                vSceneMeshZRate;
        }
    }
            ...
            switch (n) {
                case 1:
                    ...
                    for (i = 0; i < nVertexNumX + 1; i++) {
                        ...
                        try {
                            oSceneMeshBase[n].geometry.vertices[i].z =
                                oSceneMesh.geometry.vertices[nz].z;
                        } catch (e) {}
                    }
                    break;
                ...
                case 3:
                    ...
                    for (i = 0; i < nVertexNumY + 1; i++) {
                        nz =
                            (nVertexNumX + 1) * (nVertexNumY - i) + nVertexNumX;
                        oSceneMeshBase[n].geometry.vertices[i].z =
                            oSceneMesh.geometry.vertices[nz].z;
                    }
                    break;
        ...
        for (var k = 0; k < oSceneGEODataMeshArr.length; k++) {
            ...
            if (meshData.points) {
                for (var i = 0; i < meshData.points.length; i++) {
                    var p1 = _latLngToPointData(
                        viewBox,
                        meshData.points[i].lat,
                        meshData.points[i].lng,
                        meshData.points[i].h
                    );
                ...
                if (meshData['type'] == 'Text')
                    meshData.mesh.position.set(
                        p.x + meshData.iconSize.w / 2,
                        p.y - meshData.iconSize.h / 2,
                        p.z
                    );
                else meshData.mesh.position.set(p.x, p.y, p.z);
...
function SceneRender() {
    if (oRenderer != null && oScene != null && oCamera != null) {
        ...
        var radian = Math.atan2(
            oCameraCtrl.target.y - oCamera.position.y,
            oCameraCtrl.target.x - oCamera.position.x
        );
...
function mpflag(param, x, id) {
    if (!param) {
        if (id.indexOf('relief') >= 0) return 1;
        else return 0;
    }
    if (x < 1) {
        return 0;
...
function showDownloadWindow(type) {
    ...
    if (!oIconTextureCanvas) $('tr.downloadfile.icon').hide();
...
function EvtResize() {
    ...
    if (oFrame.style.display == 'block') {
        ...
        $(oDisplayInfoCheck)
            .parent()
            .css({ bottom: vFrame3D_H_Ctrl + 2 + 'px' });
...
function Download(fname, a) {
    ...
    $('.download_progress').css({
        'margin-left':
            -parseInt($('.download_progress').outerWidth() / 2) + 'px',
        'margin-top':
            -parseInt($('.download_progress').outerHeight() / 2) + 'px',
    });
...
function DownloadProc(fname, a) {
    ...
    if (fname == 'index.html') {
            ...
            .done(function (data, status, jqXHR) {
                if (data != null) {
                    ...
                    if (oGeo3DData) {
                        ...
                        for (var i = 0; i < oGeo3DData.length; i++) {
                            if (
                                oGeo3DData[i].properties._markerType ==
                                    'Polygon' ||
                                oGeo3DData[i].geometry['type'] == 'Polygon'
                            ) {
                                geo3DData.push({
                                    properties: {
                                        _markerType: 'Polygon',
                                        _fillOpacity:
                                            oGeo3DData[i].properties
                                                ._fillOpacity,
                                        _fillColor:
                                            oGeo3DData[i].properties._fillColor,
                                    ...
                                    geometry: {
                                        coordinates:
                                            oGeo3DData[i].geometry.coordinates,
                                    },
                                ...
                                for (
                                    var polyNum = 0;
                                    polyNum <
                                    oGeo3DData[i].geometry.coordinates.length;
                                    polyNum++
                                ) {
                                    var lineData = {
                                        type: 'Feature',
                                ...
                                oGeo3DData[i].properties._markerType ==
                                    'Icon' ||
                                oGeo3DData[i].geometry['type'] == 'Marker'
                            ) {
                                geo3DData.push({
                                    properties: {
                                        _markerType: 'Icon',
                                        _iconUrl:
                                            oGeo3DData[i].properties._iconUrl,
                                        _iconSize:
                                            oGeo3DData[i].properties._iconSize,
                                        _iconAnchor:
                                            oGeo3DData[i].properties
                                                ._iconAnchor,
                                        _iconScale:
                                            oGeo3DData[i].properties._iconScale,
                                    },
                                    geometry: {
                                        coordinates:
                                            oGeo3DData[i].geometry.coordinates,
                        ...
                        var geo3DDataText = JSON.stringify(
                            geo3DData,
                            undefined,
                            ' '
                        );
        ...
        if (fname == 'dem.stl') {
            data = Download_STL(vDemMesh, vZRate, vDistance);
        } else if (fname == 'dem.wrl') {
            data = Download_WRL(vDemMesh, vZRate, vDistance);
        }
    ...
    if (type == 'STL') {
        ...
        for (nY = 0; nY < colY - 1; nY++) {
            for (nX = 0; nX < colX - 1; nX++) {
                // 頂点の座標を計算
                var x1 = round(vDemXY * nX, 2);
                var y1 = round(modelSizeH + vDem[nX][nY] * vDemZ * vZRate, 2);
                var z1 = round(vDemXY * nY, 2);
                var x2 = x1;
                var y2 = round(
                    modelSizeH + vDem[nX][nY + 1] * vDemZ * vZRate,
                    2
                );
                var z2 = round(vDemXY * (nY + 1), 2);
                var x3 = round(vDemXY * (nX + 1), 2);
                var y3 = round(
                    modelSizeH + vDem[nX + 1][nY] * vDemZ * vZRate,
                    2
                );
                var z3 = z1;
                ...
                var x6 = x4;
                var y6 = round(
                    modelSizeH + vDem[nX + 1][nY + 1] * vDemZ * vZRate,
                    2
                );
                var z6 = z2;
        ...
        for (nY = 0; nY < colY - 1; nY++) {
            var x1 = 0;
            var y1 = round(modelSizeH + vDem[0][nY] * vDemZ * vZRate, 2);
            var z1 = round(vDemXY * nY, 2);
            var x2 = 0;
            var y2 = 0;
            var z2 = z1;
            var x3 = 0;
            var y3 = round(modelSizeH + vDem[0][nY + 1] * vDemZ * vZRate, 2);
            var z3 = round(vDemXY * (nY + 1), 2);
            var x4 = x3;
            var y4 = y3;
            var z4 = z3;
            var x5 = x2;
        ...
        for (nY = 0; nY < colY - 1; nY++) {
            var x1 = modelSizeX;
            var y1 = round(modelSizeH + vDem[colX - 1][nY] * vDemZ * vZRate, 2);
            var z1 = round(vDemXY * nY, 2);
            var x2 = x1;
            var y2 = 0;
            var z2 = round(vDemXY * (nY + 1), 2);
            ...
            var x5 = x1;
            var y5 = round(
                modelSizeH + vDem[colX - 1][nY + 1] * vDemZ * vZRate,
                2
            );
            var z5 = z2;
            var x6 = x1;
        ...
        for (nX = 0; nX < colX - 1; nX++) {
            var x1 = round(vDemXY * nX, 2);
            var y1 = round(modelSizeH + vDem[nX][0] * vDemZ * vZRate, 2);
            var z1 = 0;
            var x2 = round(vDemXY * (nX + 1), 2);
            var y2 = 0;
            var z2 = 0;
            var x3 = x1;
            var y3 = 0;
            ...
            var z4 = 0;
            var x5 = x2;
            var y5 = round(modelSizeH + vDem[nX + 1][0] * vDemZ * vZRate, 2);
            var z5 = 0;
            var x6 = x2;
            var y6 = 0;
            var z6 = 0;
        ...
        for (nX = 0; nX < colX - 1; nX++) {
            var x1 = round(vDemXY * nX, 2);
            var y1 = round(modelSizeH + vDem[nX][colY - 1] * vDemZ * vZRate, 2);
            var z1 = modelSizeY;
            var x2 = x1;
            var y2 = 0;
            var z2 = z1;
            var x3 = round(vDemXY * (nX + 1), 2);
            var y3 = round(
                modelSizeH + vDem[nX + 1][colY - 1] * vDemZ * vZRate,
                2
            );
            var z3 = z1;
            var x4 = x3;
        ...
        for (nY = 0; nY < colY; nY++) {
            for (nX = 0; nX < colX; nX++) {
                wrlPointList1 +=
                    '\t\t\t\t\t\t' +
                    (modelSizeX * -1 + vDemXY * nX) +
                    ' ' +
                    (modelSizeH + vDem[nX][nY] * vDemZ * vZRate) +
                    ' ' +
                    vDemXY * nY +
                    ',\n';
            }
        ...
        for (nY = 0; nY < colY; nY++) {
            wrlPointList1 +=
                '\t\t\t\t\t\t' + modelSizeX * -1 + ' 0 ' + vDemXY * nY + ',\n';
        } // ポイントリスト：左面
        for (nY = 0; nY < colY; nY++) {
            wrlPointList1 +=
                '\t\t\t\t\t\t' +
                (modelSizeX * -1 + vDemXY * (colX - 1)) +
                ' 0 ' +
                vDemXY * nY +
                ',\n';
        } // ポイントリスト：右面
        for (nX = 1; nX < colX - 1; nX++) {
            wrlPointList1 +=
                '\t\t\t\t\t\t' + (modelSizeX * -1 + vDemXY * nX) + ' 0 0,\n';
        } // ポイントリスト：奥面
        for (nX = 1; nX < colX - 1; nX++) {
            wrlPointList1 +=
                '\t\t\t\t\t\t' +
        ...
        wrlPointList1 +=
            '\t\t\t\t\t\t' +
            Math.round((modelSizeX * -1) / 2) +
            ' 0 ' +
            Math.round(modelSizeY / 2); // ポイントリスト：底面の中心点
        ...
        for (nY = 0; nY < colY; nY++) {
            for (nX = 0; nX < colX; nX++) {
                wrlPointCoordtList +=
                    '\t\t\t\t\t\t' +
                    round((vDemXY * nX) / modelSizeX, 4) +
                    ' ' +
                    round((-1 * vDemXY * nY) / modelSizeY, 4) +
                    ',\n';
                //wrlPointCoordtList += "\t\t\t\t\t\t" +  + " " + round((-1 * vDemXY * nY / modelSizeY),4) + ",\n";
            }
        }
        wrlPointCoordtList = wrlPointCoordtList.substr(
            0,
            wrlPointCoordtList.length - 2
        );
    }
    /*....................................................................*/
    return {
...
GSI.Utils.DistanceCalculator.calc = function (from, to) {
    try {
        ...
        params.lamb1 = (from.lng * PI) / 180;
        if (params.lamb1 < 0) params.lamb1 += PI * 2;
        params.phi2 = (to.lat * PI) / 180;
        params.lamb2 = (to.lng * PI) / 180;
        if (params.lamb2 < 0) params.lamb2 += PI * 2;
        ...
        if (params.c__ >= 0) {
            ...
            params.c__ < 0 &&
            params.c__ >= -Math.cos(dms2r(PI, 30000) * Math.cos(params.u1))
        ) {
            zoneInfo.zone = 2;
        ...
        if (zoneInfo.zone >= 1 && zoneInfo.zone <= 321) {
            distance = GSI.Utils.DistanceCalculator._zone1(
                PI,
                params,
                zoneInfo
            );
        } else if (zoneInfo.zone == 322) {
            distance = GSI.Utils.DistanceCalculator._zone322(
                PI,
                params,
                zoneInfo
            );
        } else if (zoneInfo.zone == 323) {
            distance = GSI.Utils.DistanceCalculator._zone323(
                PI,
                params,
                zoneInfo
            );
...
GSI.Utils.DistanceCalculator._zone1 = function (PI, params, zoneInfo) {
    ...
    for (i__ = 1; i__ <= 100; ++i__) {
        if (zoneInfo.zone == 1) {
            ...
            g = Math.sqrt(
                d__1 * d__1 * (d__2 * d__2) + d__3 * d__3 * (d__4 * d__4)
            );
            d__1 = params.etad;
            d__2 = Math.cos(params.theta / 2);
            d__3 = params.xid;
        ...
        ee =
            ...
            sgamma *
            (ssigma +
                dd * jj * (zeta + dd * kk * (d__1 * d__1 * 2 - d__2 * d__2)));
        if (zoneInfo.zone == 1) {
        ...
        d__3 = params.f;
        gg =
            params.f * (d__1 * d__1) * (1 - dd * 2 * rgamma) +
            params.f *
                zetad *
                (ssigma / zero_(jj)) *
                (1 - dd * rgamma + params.f * 0.5 * (d__2 * d__2)) +
            d__3 * d__3 * 0.25 * zeta * zetad;
        d__1 = 1 - gg;
    ...
    if (zoneInfo.zone == 1) {
        alpha = Math.atan(
            (params.xi * Math.tan(params.theta / 2)) / zero_(params.eta)
        );
        dalp2 = Math.atan(
            (params.xid * Math.tan(params.theta / 2)) / zero_(params.etad)
        );
    } else {
        alpha = Math.atan(
            (params.etad * Math.tan(params.theta / 2)) / zero_(params.xid)
        );
        dalp2 = Math.atan(
            (params.eta * Math.tan(params.theta / 2)) / zero_(params.xi)
        );
    }
    ...
    bb = (params.ep * (1 - d__1 * d__1 * 0.375)) / (d__2 * d__2);
    d__1 = rgamma;
    d__2 = zeta;
    d__3 = kk;
    d__4 = rgamma;
        ...
        (ssigma -
            bb *
                ...
                (zeta -
                    bb *
                        0.25 *
                        (kk * (d__1 * d__1 - d__2 * d__2 * 2) -
                            bb *
                                0.16666666666666666 *
                                zeta *
                                (1 - d__3 * d__3 * 4) *
                                (d__4 * d__4 * 3 - d__5 * d__5 * 4))));
    return result.s;
};
...
GSI.Utils.DistanceCalculator._zone3 = function (PI, params, zoneInfo) {
    ...
    zoneInfo.rr =
        ...
        (d__1 * d__1) *
        (1 -
            params.f * 0.25 * (params.f + 1) * (d__2 * d__2) +
            d__3 * d__3 * 0.1875 * (d__4 * d__4));
    ...
    zoneInfo.f1 = params.f * 0.25 * (params.f * 0.5 + 1);

    d__1 = zoneInfo.q;
    zoneInfo.gamma0 =
        zoneInfo.q +
        zoneInfo.f1 * zoneInfo.q -
        zoneInfo.f1 * (d__1 * (d__1 * d__1));
...
GSI.Utils.DistanceCalculator._zone31 = function (PI, params, zoneInfo) {
    ...
    k =
        ((zoneInfo.f1 + 1) *
            Math.abs(params.sigmad) *
            (1 - params.f * params.y)) /
        (params.f * PI * zero_(params.y));
    d__1 = cos(psi);
    j1 = j / (k / zero_(d__1) + 1);
    psid = Math.asin(j1);
    ...
    params.theta =
        Math.atan(
            (Math.tan((psid + psidd) / 2.0) *
                Math.sin(Math.abs(params.sigmad) / 2.0)) /
                zero_(d__1)
        ) * 2;
    return 0;
};
...
/*************************************************
 経度差0度の方位角の判断
*************************************************/
GSI.Utils.DistanceCalculator._handan1 = function (
    PI,
    params,
    zoneInfo,
    result
...
/*************************************************
 経度差180度の方位角の判断
*************************************************/
GSI.Utils.DistanceCalculator._handan2 = function (
    PI,
    params,
    zoneInfo,
    result
...
GSI3D.LayersJSONLoader.prototype.load = function () {
    ...
    for (var i = 0; i < this._list.length; i++) {
        ...
        if (!this._requests[url]) this._requests[url] = { url: url };
    }
...
GSI3D.LayersJSONLoader.prototype._onLoaded = function (url, json) {
    ...
    if (json.layers) this._initLayersJSON(this._hash, url, json.layers);
};

// LayersJSON解析
GSI3D.LayersJSONLoader.prototype._initLayersJSON = function (
    result,
    url,
    list
) {
...
GSI3D.DEMManager.prototype.load = function () {
    for (var key in this._loaders) {
        ...
        loader.loader = new GSI3D.DEMLoader(
            loader.coords.x,
            loader.coords.y,
            loader.coords.z,
...
GSI3D.DEMLoader.prototype.getHillshademapImage = function () {
    return this._hillshademapLoaded && !this._hillshademapError
...
GSI3D.DEMLoader.prototype.load = function () {
    ...
    this._urlList = GSI3D.DEMLoader.getURLList(
        this._coords.x,
        this._coords.y,
        this._coords.z
    );
            ...
            GSI3D.bind(
                function (coords) {
                    ...
                    if (this._coords.z != coords.z) {
                        ...
                        rb = L.point(
                            (coords.x + 1) * 256 * scale,
                            (coords.y + 1) * 256 * scale
                        );
...
GSI3D.DEMLoader.prototype._nextZoom = function () {
    ...
    var point = GSI3D.point(
        (this._coords.x * 256) / scale,
        (this._coords.y * 256) / scale
    )
        .divideBy(256)
        .floor();
...
GSI3D.DEMLoader.prototype._loadDEM = function (urlList, coords) {
    ...
    if (!targetUrl) {
        //err
        if (this.options.overZooming) this._nextZoom();
        else {
            this._demLoadError();
        }
        return;
...
// 標高データ取得後データ解析
GSI3D.DEMLoader.prototype._demLoadSuccess = function (
    urlList,
    coords,
    targetUrl
) {
...
GSI3D.DEMLoader.getURLList = function (x, y, z) {
    if (!GSI3D.DEMLoader.DEMAREA) {
        ...
        for (var i = 0; i < CONFIG.DEMAREA.length; i++)
            GSI3D.DEMLoader.DEMAREA[CONFIG.DEMAREA[i]] = 1;
        for (var i = 0; i < CONFIG.DEMAREA2.length; i++)
            GSI3D.DEMLoader.DEMAREA2[CONFIG.DEMAREA2[i]] = 1;
        for (var i = 0; i < CONFIG.DEMAREA3.length; i++)
            GSI3D.DEMLoader.DEMAREA3[CONFIG.DEMAREA3[i]] = 1;
        delete CONFIG.DEMAREA;
        CONFIG.DEMAREA = null;
        delete CONFIG.DEMAREA2;
        CONFIG.DEMAREA2 = null;
        delete CONFIG.DEMAREA3;
        CONFIG.DEMAREA3 = null;
    }

    //-------------------------------------------------------------------------------
    var getCoords = function (x, y, z, targetZoom) {
        var scale = Math.pow(2, z - targetZoom);
        var point = GSI3D.point((x * 256) / scale, (y * 256) / scale)
            .divideBy(256)
            .floor();
    ...
    var coordsToKey = function (coords) {
    ...
    if (!GSI3D.DEMLoader.DEMAREA[key])
        return [
            {
                url:
                    'https://cyberjapandata.gsi.go.jp/xyz/demgm_png/{z}/{x}/{y}.png',
...
GSI3D.ReliefTileLayer.decodeElevationDataText = function (txt) {
    ...
    try {
        ...
        for (var i = 0; i < parts.length; i += 2) {
            ...
            if (parts[i] == '') item.h = null;
            else item.h = parseInt(parts[i], 16);

            if (parts[i + 1] == '') item.color = null;
            else
                item.color = GSI3D.ReliefTileLayer.colorStringToRGBA(
                    '#' + parts[i + 1]
                );
...
GSI3D.ReliefTileLayer.TileDrawer.prototype.setElevationData = function (
    elevationData
) {
    this._elevationData = this._initializeElevationData(elevationData);
    if (!this._elevationData)
        this._elevationData = this._initializeElevationData(
            GSI3D.ReliefTileLayer.getElevationSampleData()
        );
};
...
// 色別標高図のデータを整理
GSI3D.ReliefTileLayer.TileDrawer.prototype._initializeElevationData = function (
    data
) {
...
// 色別標高図のタイルに描画
GSI3D.ReliefTileLayer.TileDrawer.prototype.draw = function (
    dstCanvas,
    demData,
    hillshadeMapImage
) {
    ...
    var idx = 0,
        ...
        hillshadeIdx = 0,
        color,
        hillshadeColor = { r: 0, g: 0, b: 0, a: 0 };
    for (var y = 0; y < 256; ++y) {
        for (var x = 0; x < 256; ++x) {
            ...
            if (color) {
                if (hillshadeData) {
                    if (hillshadeScale != 1) {
                        var x2 = Math.floor(
                            (hillshadePoint.x + x) / hillshadeScale
                        );
                        var y2 = Math.floor(
                            (hillshadePoint.y + y) / hillshadeScale
                        );
                        hillshadeIdx = y2 * 256 * 4 + x2 * 4;
                    } else hillshadeIdx = y * 256 * 4 + x * 4;
					...
					*/
                    if (hillshadeColor.a > 0) {
                        destData.data[destIdx] = Math.round(
                            color.r * (hillshadeColor.r / 255)
                        );
                        destData.data[destIdx + 1] = Math.round(
                            color.g * (hillshadeColor.g / 255)
                        );
                        destData.data[destIdx + 2] = Math.round(
                            color.b * (hillshadeColor.b / 255)
                        );
                        destData.data[destIdx + 3] = Math.round(
                            color.a * (hillshadeColor.a / 255)
                        );
                    } else {
...
GSI3D.ReliefTileLayer.TileDrawer.getCanvas = function () {
    if (!GSI3D.ReliefTileLayer.TileDrawer._canvas) {
        GSI3D.ReliefTileLayer.TileDrawer._canvas = document.createElement(
            'canvas'
        );
